...
from cgcnn.data_PU_learning import collate_pool, split_bagging, bootstrap_aggregating
from cgcnn.model_PU_learning import CrystalGraphConvNet

parser = argparse.ArgumentParser(description='NCR prediction by PU-learning using CGCNN classifier')
...
def main():
    ...
    for bagging in range(args.restart, args.restart+args.bag):
        ...
        best_checkpoint = torch.load('checkpoint_bag_'+str(bagging+1)+'.pth.tar', weights_only=False)
    ...
    if prediction.shape[1] == 2:
        ...
        if not test:
            if len(np.unique(target_label)) < 2:
                auc_score = 0.0
                print("Warning: Only one class in target. Skipping AUC computation.")
            else:
