    ...
    if os.path.exists(pickle_folder+'/'+cif_id+'.pickle'):
        ...
        try:
            ...
            with warnings.catch_warnings():
                ...
                atoms = read(os.path.join(root_dir,cif_id+'.cif'))
                write(os.path.join(root_dir,cif_id+'.cif'), atoms)
                crystal = Structure.from_file(os.path.join(root_dir,cif_id+'.cif'))
...
def safe_preprocess(data):
    try:
        ...
        return {'failed': data[0], 'error': str(e)}
...
if __name__ == '__main__':
    ...
    with open(id_prop_file, encoding='utf-8-sig') as g:
        reader = csv.reader(g)
    ...
    fail_log_path = os.path.join(pickle_folder, 'fail.txt')

    fail_log_path = os.path.join(pickle_folder, 'fail.txt')
    with open(fail_log_path, 'w') as fail_log:
        for result in tqdm(mapper(safe_preprocess, id_prop_data), total=len(id_prop_data)):
            if isinstance(result, dict) and 'failed' in result:
                fail_log.write(f"{result['failed']}\tError: {result.get('error', 'Unknown error')}\n")
                fail_log.flush()
                continue
